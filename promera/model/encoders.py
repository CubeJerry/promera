...
class SingleConditioning(Module):
    ...
    def __init__(
        ...
        input_dim = 2 * token_s + 2 * _ntoks + 2
    ...
    def forward(
        ...
        if model_cache is None or len(layer_cache) == 0:
            ...
            atom_feats = [
                ...
                torch.nn.functional.one_hot(feats["ref_element"], 128),
            ]
            name_chars = torch.nn.functional.one_hot(
                feats["ref_atom_name_chars"], 64
            ).reshape(B, N, 4 * 64)

            name_chars *= feats["atom_is_std"][..., None]  # only use name chars if std
            atom_feats.append(name_chars)
            atom_feats.append(torch.nn.functional.one_hot(feats["ref_hydrogens"], 8))
            ...
            if self.structure_prediction:
                ...
                atom_to_token = (
                    torch.nn.functional.one_hot(feats["atom_to_token"], M).float()
...
class AtomAttentionDecoder(Module):
    ...
    def __init__(
        ...
        if has_alt_update:
            self.atom_feat_to_atom_pos_update_alt = nn.Sequential(
                nn.LayerNorm(atom_s), LinearNoBias(atom_s, 3)
            )
            init.final_init_(self.atom_feat_to_atom_pos_update_alt[1].weight)
    ...
    def forward(
        ...
        atom_to_token = (
            torch.nn.functional.one_hot(
                feats["atom_to_token"], a_to_q.shape[-2]
            ).float()
            * feats["atom_pad_mask"][..., None]
        )
        atom_to_token = atom_to_token.repeat_interleave(multiplicity, 0)
        ...
        if hasattr(self, "atom_feat_to_atom_pos_update_alt"):
            r_update_alt = self.atom_feat_to_atom_pos_update_alt(q)
            return r_update, r_update_alt
        else:
            return r_update
