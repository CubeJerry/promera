...
def _map_epitope_residues(schema: dict, cfg, epitope_chain: str) -> list:
    """Convert target-template residue IDs to schema positions for cropped targets.

    Historically epitope_residues were interpreted as 1-based positions in the
    input schema sequence. When target_template is present, users often pass
    residue IDs from that template instead. If every requested residue exists as
    a template residue ID, map those IDs through the schema-template alignment
    and return schema 1-based positions. If no requested residue is present in
    the template, preserve the historical schema-position behavior.
    """
